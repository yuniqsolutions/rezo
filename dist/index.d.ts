...
import { Cookie as TouchCookie, CookieJar as TouchCookieJar, CreateCookieJarOptions, CreateCookieOptions, ErrorCallback as ErrorCallback$1, MemoryCookieStore, Nullable, Store } from 'tough-cookie';
...
export interface RezoCookieStoreOptions {
	/** Maximum number of cookies per domain (0 = unlimited) */
	maxCookiesPerDomain?: number;
	/** Maximum total cookies in the store (0 = unlimited) */
	maxTotalCookies?: number;
	/** Auto-purge expired cookies on put operations */
	autoPurge?: boolean;
	/** Callback fired when store changes */
	onChange?: (event: CookieStoreEvent) => void;
}
export interface CookieStoreEvent {
...
export interface CookieStoreStats {
	...
	/** Number of session cookies (no expiry) */
...
export declare const CookieJar: typeof RezoCookieJar;
/**
...
/**
 * Request interceptor manager — wraps hooks.beforeRequest
 *
 * Each `use(fulfilled, rejected)` call:
 * - Pushes a beforeRequest hook that calls `fulfilled(config)` and returns the modified config
 * - If `rejected` is provided, errors from previous hooks are caught and passed to it
 */
...
/**
 * Response interceptor manager — wraps hooks.afterResponse and hooks.beforeError
 *
 * Each `use(fulfilled, rejected)` call:
 * - Pushes an afterResponse hook that calls `fulfilled(response)`
 * - If `rejected` is provided, pushes a beforeError hook that calls `rejected(error)`
 */
...
/**
 * RezoUri — extends the native URL with convenience properties.
 *
 * Works everywhere URL or string is accepted (toString/href).
 * Adds `.params`, `.auth`, `.toObject()`, and `.clone()`.
 */
export declare class RezoUri extends URL {
	/**
 ...
 *
 * Returns a callable function with all HTTP methods attached.
 * The instance can be invoked directly as a function or via named methods.
 *
 * @param adapter - The HTTP adapter function to use for requests
 * @param config - Optional default configuration for all requests
 * @returns A callable Rezo instance
 *
 * @example
 * ```typescript
 * // Direct invocation (defaults to GET)
 * const response = await rezo('https://api.example.com/users');
 *
 * // With options
 * const response = await rezo('https://api.example.com/users', {
 ...
 * const created = await rezo.postJson('https://api.example.com/users', { name: 'John' });
 * ```
 */
...
export type RezoMemoryCookieStoreIndex = {
	[domain: string]: {
		[path: string]: {
			[key: string]: Cookie;
		};
...
declare class RezoMemoryCookieStore extends MemoryCookieStore {
	idx: RezoMemoryCookieStoreIndex;
}
export declare class RezoCookieStore extends RezoMemoryCookieStore {
	private options;
	constructor(options?: RezoCookieStoreOptions);
	putCookie(cookie: Cookie): Promise<void>;
	putCookie(cookie: Cookie, callback: ErrorCallback$1): void;
	updateCookie(_oldCookie: Cookie, newCookie: Cookie): Promise<void>;
	updateCookie(_oldCookie: Cookie, newCookie: Cookie, callback: ErrorCallback$1): void;
	removeCookie(domain: string, path: string, key: string): Promise<void>;
	removeCookie(domain: string, path: string, key: string, callback: ErrorCallback$1): void;
	removeAllCookies(): Promise<void>;
	removeAllCookies(callback: ErrorCallback$1): void;
	/** Get all domains that have cookies stored */
	getDomains(): string[];
	/** Get all cookies for a specific domain (all paths) */
	getCookiesForDomain(domain: string): Cookie[];
	/** Count cookies for a specific domain */
	countCookiesForDomainSync(domain: string): number;
	/** Count all cookies in the store */
	countAllSync(): number;
	/** Check if store has any cookies for the domain */
	hasDomain(domain: string): boolean;
	/** Find a specific cookie by domain, path, and key (sync) */
	findCookieSync(domain: string, path: string, key: string): Cookie | undefined;
	/** Remove all expired cookies from the store. Returns number of cookies purged. */
	purgeExpiredSync(): number;
	/** Get all expired cookies without removing them */
	getExpiredCookies(): Cookie[];
	/** Evict the oldest cookie for a domain (by creation time) */
	private evictOldestForDomainSync;
	/** Evict the oldest cookie across all domains */
	private evictOldestSync;
	/** Get detailed stats about the cookie store */
	getStats(): CookieStoreStats;
	/** Remove all cookies for a specific domain. Returns number of cookies removed. */
	removeCookiesForDomainSync(domain: string): number;
	/** Export all cookies as a flat array */
	exportCookiesSync(): Cookie[];
	/** Import cookies from a flat array directly into the store */
	importCookiesSync(cookies: Cookie[]): void;
	private emitChange;
}
/**
 ...
 * @example
 * ```ts
 * parseLinkHeader('<https://api.example.com/users?page=2>; rel="next", <https://api.example.com/users?page=5>; rel="last"')
 * // → { next: 'https://api.example.com/users?page=2', last: 'https://api.example.com/users?page=5' }
 * ```
 */
export declare function parseLinkHeader(header: string | null | undefined): Record<string, string>;
