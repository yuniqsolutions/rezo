...
/**
 * Limiter queue configuration options for rate limiting.
 * @description Configuration options for RezoQueue instances used as crawler limiters.
 *
 * Includes the limiter-specific `randomDelay` field for per-domain jitter, which
 * is why this is distinct from the base `queueOptions` (aliased to `QueueConfig`)
 * exported from `types/options.ts`.
 */
export interface LimiterQueueOptions {
...
export type OxylabsGeoLocation = keyof typeof OXYLABS_OPTIONS.geo_location.options;
export type OxylabsLocale = keyof typeof OXYLABS_OPTIONS.locale.options;
export type OxylabsBrowserType = keyof typeof OXYLABS_OPTIONS.user_agent_type.options;
export interface OxylabsOptions {
	username: string;
	password: string;
	browserType?: OxylabsBrowserType | string;
	locale?: OxylabsLocale | string;
	geoLocation?: OxylabsGeoLocation | string;
...
export type DecodoGeoLocation = keyof typeof DECODO_OPTIONS.geo_location.options;
export type DecodoLocale = keyof typeof DECODO_OPTIONS.locale.options;
export type DecodoBrowserType = keyof typeof DECODO_OPTIONS.user_agent_type.options;
export type DecodoHeadlessMode = "html" | "png" | "pdf";
export interface IOptions {
	username: string;
	password: string;
	browserType?: DecodoBrowserType | string;
	locale?: DecodoLocale | string;
	geoLocation?: DecodoGeoLocation | string;
	http_method?: "get" | "post";
	base64Body?: string;
	successful_status_codes?: number[];
	session_id?: string;
	...
	deviceType?: DecodoBrowserType | string;
	country?: DecodoGeoLocation | string;
	state?: string;
	city?: string;
...
export interface StableThroughputPresetOptions {
	...
	globalLimiter?: LimiterQueueOptions | false;
	...
	domainLimiter?: LimiterQueueOptions | false;
		...
		labs: [
			{
				...
				queueOptions: LimiterQueueOptions;
			}
		];
	} | {
		enable: false;
...
/**
 * Redirect event data passed to onRedirect handler
 */
export interface CrawlerRedirectEvent {
	originalUrl: string;
	finalUrl: string;
	redirectCount: number;
	statusCode: number;
...
export declare class Crawler {
	 ...
	 * ```
	 */
	onRedirect(handler: (event: CrawlerRedirectEvent) => Promise<void>): Crawler;
	/**
