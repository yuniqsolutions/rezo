 ...
 */
export type AdapterType = "http" | "http2" | "curl" | "fetch" | "xhr" | "react-native";
/**
...
export interface AdapterCapabilities {
	/** Supports tough-cookie jar integration */
	cookies: boolean;
	...
	uploadProgress: boolean;
	/** Supports download progress events */
	downloadProgress: boolean;
	/** Supports file downloads to disk */
	fileDownload: boolean;
	...
	tlsConfig: boolean;
	/** Supports React Native background task integration */
	backgroundTasks: boolean;
}
/**
 * Adapter selection context
 */
export interface AdapterContext {
	/** Request requires cookie jar integration */
	needsCookies: boolean;
	/** Request requires proxy support */
	needsProxy: boolean;
	/** Request is a streaming response */
	needsStreaming: boolean;
	/** Request should use HTTP/2 */
	needsHttp2: boolean;
	/** Request needs upload progress tracking */
	needsUploadProgress: boolean;
	/** Request needs download progress tracking */
	needsDownloadProgress: boolean;
	/** Request needs file download */
	needsFileDownload: boolean;
	/** Request needs React Native background task integration */
	needsBackgroundTasks: boolean;
	/** Optional React Native provider capabilities configured by the app */
	reactNative?: {
		hasFileSystemAdapter: boolean;
		hasStreamTransport: boolean;
		hasFileDownloadSupport: boolean;
		hasDownloadProgressSupport: boolean;
		hasFileUploadSupport: boolean;
		hasUploadProgressSupport: boolean;
		hasNetworkInfoProvider: boolean;
		hasBackgroundTaskProvider: boolean;
	};
	/** User-preferred adapter (if specified) */
...
export interface AdapterModule {
	executeRequest<T = any>(options: RezoRequestConfig, defaultOptions: RezoDefaultOptions, jar: RezoCookieJar): Promise<RezoResponse<T> | RezoStreamResponse | RezoDownloadResponse | RezoUploadResponse>;
}
/**
 * Detect the current runtime environment
 * @returns RuntimeEnvironment object with detection flags
 */
export declare function detectRuntime(): RuntimeEnvironment;
/**
 * Get adapter capabilities matrix
 * @param adapter - Adapter type to check
 * @returns AdapterCapabilities object
 */
export declare function getAdapterCapabilities(adapter: AdapterType): AdapterCapabilities;
/**
 * Build adapter context from request configuration
 * @param options - Request configuration
 * @param defaultOptions - Default options
 * @returns AdapterContext for selection
 */
export declare function buildAdapterContext(options: RezoRequestConfig, defaultOptions: RezoDefaultOptions): AdapterContext;
/**
 * Get available adapters for the current runtime
 * @param runtime - Current runtime environment
 * @returns Array of available adapter types, ordered by preference
 */
export declare function getAvailableAdapters(runtime: RuntimeEnvironment): AdapterType[];
/**
 * Select the optimal adapter for the given context
 * @param context - Request context
 * @param runtime - Runtime environment (optional, auto-detected if not provided)
 * @returns Selected adapter type
 */
export declare function selectAdapter(context: AdapterContext, runtime?: RuntimeEnvironment): AdapterType;
/**
 * Dynamically load and cache an adapter module
 * @param adapter - Adapter type to load
 * @returns Promise resolving to adapter module
 */
export declare function loadAdapter(adapter: AdapterType): Promise<AdapterModule>;
/**
 * Clear the adapter cache (useful for testing or hot-reloading)
 */
export declare function clearAdapterCache(): void;
/**
 * Get diagnostic information about adapter selection
 * Useful for debugging adapter selection issues
 * @param options - Request configuration
 * @param defaultOptions - Default options
 * @returns Diagnostic information
 */
export declare function getAdapterDiagnostics(options: RezoRequestConfig, defaultOptions: RezoDefaultOptions): {
	runtime: RuntimeEnvironment;
	context: AdapterContext;
	available: AdapterType[];
	selected: AdapterType;
	scores: Array<{
		adapter: AdapterType;
		score: number;
	}>;
	selectedCapabilities: AdapterCapabilities;
};
/**
 * Adapter picker configuration for Rezo instance
 */
export interface AdapterPickerConfig {
	/** Manually specify adapter to use */
	adapter?: AdapterType;
	/** Custom adapter selection function */
	customPicker?: (context: AdapterContext, runtime: RuntimeEnvironment) => AdapterType;
	/** Enable debug logging for adapter selection */
	debug?: boolean;
}
/**
 * Create an adapter picker function with custom configuration
 * @param config - Picker configuration
 * @returns Configured picker function
 */
export declare function createAdapterPicker(config?: AdapterPickerConfig): (options: RezoRequestConfig, defaultOptions: RezoDefaultOptions) => AdapterType;
/**
